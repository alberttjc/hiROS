...
class ReadValidImagesAndActionTypesByTxt(object):
    ...
    def __init__(self, img_folder, valid_imgs_txt,
        ...
        print("Reading images from txtscript: {}".format(img_folder))
        print("Reading images information from: {}".format(valid_imgs_txt))
    ...
    def read_image(self):
        ...
        if self.i > len(self.images_info):
            raise RuntimeError("There are only {0} images, \n but you try to read the {1}th image".format(len(self.images_info),self.i))
...
def load_skeleton_data(filepath, classes):
    ...
    with open(filepath, 'r') as f:
        ...
        P = len(X[0])
        C = len(classes)
        #print("\nNumber of samples = {0} \n Raw feature length = {1} \n Number of classes = {2}".format(N,P,C))
        #print(f"Classes: {classes}".format)
